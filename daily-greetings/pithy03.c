...
// 2. The list_base is a pointer-pointer variable. It is allocated enough stoarge to hold 100 entries. The typcast (char **) matches the variable's data type.
// 3. Using the offset prvided by the items count, this statement copies the address store in pointer variable entry into the list maintained at location list_base. Only the address is copied, not the whole string.
